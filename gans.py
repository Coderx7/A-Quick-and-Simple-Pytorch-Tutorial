...
def display_images(imgs, cols=8, title='',unnormalize=False, save_path=None, figsize=(12,6)):
    ...
    plt.title(title)
    #update from future: 
    # in case plt.gird() is applied in a previous plot() invocation()
    plt.grid(False)
...
def plot_loss(history:dict, loss_type, figsize=(12,6)):
    plt.style.use('ggplot')
    
    fig, (ax1,ax2) = plt.subplots(1,2,figsize=figsize)
    epochs = range(len(list(history.values())[0]))
    
    ax1.plot(epochs, history["dloss"], label="Discriminator", color="tab:blue", alpha=0.7)
    ax1.plot(epochs, history["gloss"], label="Generator", color="tab:cyan", alpha=0.7)#linewidth=2
    ...
    ax2.plot(epochs, history["dreal_scores"], label="D Real Scores", color="tab:green", alpha=0.7)
    ax2.plot(epochs, history["dfake_scores"], label="D Fake Scores", color="tab:brown", alpha=0.7)
    
    ax2.set_title("Average Disc Scores")
    ax2.set_xlabel("Epochs")
    ax2.set_ylabel(f"Avg-scores")
    ax2.legend()
    ax2.grid(True)
...
# dc = {"dloss":torch.sin(torch.randn((10,))).tolist(),
#       "gloss":torch.cos(torch.randn((10,))).tolist(),
#       "real_scores":torch.asin(torch.randn((10,))).tolist(),
#       "fake_scores":torch.acos(torch.randn((10,))).tolist()}
# print(list(dc.values())[0])
# print(dc["dloss"])
# plot_loss(dc,'wgan')
...
def training_loop(discriminator, generator, train_loader, disc_optimizer:torch.optim.Adam, gen_optimizer,
    ...
    history = {"dloss":[], "gloss":[], "dreal_scores":[], "dfake_scores":[]}
    ...
    for epoch in range(epochs):
        ...
        # losses = []
        # epoch_scores = []
        epoch_losses = {"dloss":[], "gloss":[], "dreal_scores":[], "dfake_scores":[]}
        for i, (imgs_real, _) in enumerate(train_loader):
            #scale input to [-1,1]
            imgs_real = (2*imgs_real-1).to(device)
        ...
        epoch_losses["dloss"].append(disc_loss.item())
        epoch_losses["gloss"].append(gen_real_loss.item())
        epoch_losses["dreal_scores"].append(disc_real_mean)
        epoch_losses["dfake_scores"].append(disc_fake_mean)
        ...
        # average each epoch worth of loss/scores to track the changes during training
        avgs = {k:np.mean(np.array(v)) for k,v in epoch_losses.items()}
        
        history["dloss"].append(avgs["dloss"])
        history["gloss"].append(avgs["gloss"])
        history["dreal_scores"].append(avgs["dreal_scores"])
        history["dfake_scores"].append(avgs["dfake_scores"])
        ...
        print(f" -- Epoch's Avg: Disc's real mean: {avgs["dreal_scores"]:.4f} | Disc's fake mean: {avgs["dfake_scores"]:.4f}")
        print(f'[Epoch {epoch}/{epochs}] Disc Loss-Avg: {avgs["dloss"]:.6f} | Gen loss-Avg: {avgs["gloss"]:.6f} | IS: (μ:{IS_score[0]:.4f}, σ²:{IS_score[1]:.4f}) | FID: {FID_score:.2f}')
        ...
        torch.save({"state_dict":generator.state_dict(),
                ...
                "FID":FID_score,
                "IS":IS_score,
                "d_loss_mean":avgs["dloss"],
                "g_loss_mean":avgs["gloss"],
        ...
        with torch.no_grad():
            ...
            display_images(generated_images, 
                    cols=gen_num_samples//8,
                    title=f'Using {loss_type.upper()} at Epoch {epoch} FID:{FID_score:.2f} (dLoss:{avgs["dloss"]:.6f} | gLoss:{avgs["gloss"]:.6f})',
                    unnormalize=True,
...
# so we are getting an incorrect slower lower frequency wave! which we call frequency aliasing!
# (if we go back to pixel space, it means since the pixel grid only samples once per pixel,
# it can not capture sub pixel waves!hence we lose information like that! 
# I guess its obvious at this point!)
# 
# (recap2:
...
# above that, gets wraped back into the valid range!
# just like the modulo operation but in frequency space! so to recap, the frequency is too high
...
# because of these new changes, the styleganv3 is a lot slower than the previous version!
# 
# the discriminator stays the same, the losses, basically everything else stays the same except the
# generator that goes throuhg the changes we talked about
# 
# recap:
...
# 
# so the stylegan3 
# removed all aliasing sources
# replaced noise injections
# replaced upsampling ops
# replaced modulated convs with filtered versions (added fixed lowpass filters)
# stablized high frequency with signal processing constraints
...
#Update:
# this is a completely wrong implementation and Im going to leave it for now
# I couldnt do any simplification, to get the good result we need tdo what the
# original impl does and its really complicated! 
# I found out the fourier input needs to apply transformation on w. if we dont
# (check first version) the features like lips, eyes, etc wont be placed at their
# right location with the right angle! e.g. when we train if you look closely you'll see
# there are sevral lips, 90 degress roatated at different locations, this the case for nose
# and eye balls, so we added the transformation matrix to fourier input. but then
# we see the frequencies cutoff/ related stuff are not accurate and the resulting images
# are like broken glass!
# it took a lot of time so I want to leave it as is. check the jypternotebooks of gans_debugging_stylegan3_fixedtraining1
# and gans_debugging_stylegan3_newfailedtest2 to see my previous experiments
# the gans_debugging_stylegan3_newfailedtest2 containsthe fourier input I copied from the
# official implementation, but as you can seebecause its half assed! it fails spectecularly!
# so I gave up for now! as its really a dead arch, and its too specific, engineered. we get
# way better results using diffusion! 
...
class FourierInput00(nn.Module):
    def __init__(self, channels, dim_size=4, scale=2):
        super().__init__()
        self.dim_size = dim_size
        self.channels = channels
...
class StyleConvBlock3(nn.Module):
    ...
    def forward(self, x, w):
        ...
        # this shouldnt be here! but I placed it here to have a faster training 
        # upsample
        if self.up>1:
            # we can now experiment with the old method as well     
...
class GeneratorStyleGAN3(nn.Module):
    ...
    def setup_layers(self, z_size, w_size, mn_num_layers, channels,
        ...
        self.fourier_input = FourierInput(self.channels[0],dim_size=4, w_size=w_size,scale=self.scale)
...
gen = GeneratorStyleGAN3(100,100,channels=channels)
...
betas = [0, 0.99]
# 0.003
lr_d = 0.002 if not use_fp16 else 0.001
lr_g = 0.002 if not use_fp16 else 0.001
...
training_loop_stylegan3(discriminator_stylegan3,
                     ...
                     quick_and_noisy_IS_FID=False,
                     experimental_size=experimental_size,
                     style_mix_epoch_start=style_mix_epoch_start,
                     )
...
# 
# I gave up! I cant simply implement this like this with simplifications, need to understand it completely
# and then follow the exact constraints like the official impl which I dont have the time for! 
...
def display_image_grid(real_imgs, fake_imgs, nrows=None, ncols=None, title='', figsize=(12,6), save_path=None):
    ...
    for i, (img_r, img_f) in enumerate(zip(real_imgs, fake_imgs)):
        ax = fig.add_subplot(nrows, ncols, i+1, xticks=[],yticks=[])
        # put real image next to its fake counterpart
        img_pair = torch.cat([img_r, img_f], dim=-1)
        img_pair = unnormalize(img_pair).transpose(1,2,0)
        ax.imshow(img_pair)
...
def get_yosemite_dataloaders(is_test=False, batch_size = 16, resize_dim = (128,128), drop_last=False):
    ...
    dataset_size = min(get_dataset_size('yosemite-summer'),
                       get_dataset_size('yosemite-winter'))
    ...
    dataloaders = []
    for name in names:
        dl = get_dataloader(dataset_name=name, 
...
dataloader_summer, dataloader_winter = get_yosemite_dataloaders(is_test=False, 
                                                                batch_size=batch_size,
                                                                resize_dim=resize_dim,
                                                                drop_last=True)

# lets read some test images and use them for testing our network
test_dataloader_summer, test_dataloader_winter = get_yosemite_dataloaders(is_test=True,
                                                                          batch_size=batch_size,
                                                                          resize_dim=resize_dim)
