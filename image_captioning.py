...
# 
# # side note:
# Causal LLM vs Seq2seq llm: 
# **Causal Language Modeling (CLM)** and **Sequence-to-Sequence (Seq2Seq)** are two different approaches used in training language models¹².
# **Causal Language Modeling (CLM)**:
# - CLM is an autoregressive method where the model is trained to predict the next token in a sequence given the previous tokens¹.
# - CLM is used in models like GPT-2 and GPT-3 and is well-suited for tasks such as text generation and summarization¹.
...
# - This approach is commonly used in tasks like machine translation, summarization, and question-answering¹.
...
# In summary, the key difference between CLM and Seq2Seq lies in their training approach and the types of tasks they are suited for¹². CLM models are typically used for tasks that involve predicting the next token in a sequence, while Seq2Seq models are used for tasks that involve transforming an input sequence into an output sequence¹². Let me know if you need further assistance! 😊
# Source: Conversation with Bing, 1/23/2024
# (1) Understanding Causal LLM’s, Masked LLM’s, and Seq2Seq: A ... - Medium. https://medium.com/@tom_21755/understanding-causal-llms-masked-llm-s-and-seq2seq-a-guide-to-language-model-training-d4457bbd07fa.
# (2) Causal LLMs and Seq2Seq Architectures | Niklas Heidloff. https://heidloff.net/article/causal-llm-seq2seq/.
# (3) Causal language modeling - Hugging Face. https://huggingface.co/docs/transformers/tasks/language_modeling.

# sidenote2:
# Image captioning is typically considered a Sequence-to-Sequence (Seq2Seq) task. 
# In image captioning, the input is an image and the output is a sequence of words
# (the caption). The model needs to transform the input image features into a coherent 
# sequence of words². This is different from a Causal Language Modeling (CLM) task, 
# where the model predicts the next word in a sequence based on the previous words.
# In the context of image captioning, the model often employs an encoder-decoder architecture,
# which is a characteristic of Seq2Seq models². The encoder processes the image and extracts
# relevant features, and the decoder generates the caption based on these features.
# However, it's worth noting that the boundaries between different types of models 
# can sometimes be blurred, and the specific implementation can vary depending on the
# task and the approach taken.
# 
# refs 1/23/2024
# Dependent Multi-Task Learning with Causal Intervention for Image Captioning. https://arxiv.org/abs/2105.08573.
# Causal LLMs and Seq2Seq Architectures | Niklas Heidloff. https://heidloff.net/article/causal-llm-seq2seq/.
# https://doi.org/10.48550/arXiv.2105.08573.


# copied from Understanding Causal LLM’s, Masked LLM’s, and Seq2Seq: A ... - Medium. https://medium.com/@tom_21755/understanding-causal-llms-masked-llm-s-and-seq2seq-a-guide-to-language-model-training-d4457bbd07fa.
# Causal Language Modeling (CLM)
# CLM is an autoregressive method where the model is trained to predict the next token in a 
# sequence given the previous tokens. CLM is used in models like GPT-2 and GPT-3 and is 
# well-suited for tasks such as text generation and summarization. 
# However, CLM models have unidirectional context, meaning they only consider the past and 
# not the future context when generating predictions.
# 
# Masked Language Modeling (MLM)
# MLM is a training method used in models like BERT, where some tokens in the input sequence 
# are masked, and the model learns to predict the masked tokens based on the surrounding 
# context. MLM has the advantage of bidirectional context, allowing the model to consider 
# both past and future tokens when making predictions. This approach is especially useful 
# for tasks like text classification, sentiment analysis, and named entity recognition.
#
# Sequence-to-Sequence (Seq2Seq)
# Seq2Seq models consist of an encoder-decoder architecture, where the encoder processes the
# input sequence and the decoder generates the output sequence. This approach is commonly used
# in tasks like machine translation, summarization, and question-answering. 
# Seq2Seq models can handle more complex tasks that involve input-output transformations, 
# making them versatile for a wide range of NLP tasks.
#
# Key Differences in CLM, MLM, Seq2Seq
# key differences in the implementation, architecture, and output models for causal language 
# modeling (CLM), masked language modeling (MLM), and sequence-to-sequence (seq2seq) modeling.
# Causal Language Modeling (CLM):
#     Implementation: In CLM, the model is trained to predict the next token in the sequence, 
#                     given the previous tokens. During training, the input tokens are fed into
#                     the model, and the model predicts the probability distribution of the next 
#                     token. The loss is calculated based on the model’s predictions and the 
#                     actual target tokens, which are just the input tokens shifted by one position.
#     Architecture: CLM is typically used with autoregressive models like GPT. These models use 
#                   a unidirectional (left-to-right) Transformer architecture, where each token 
#                   can only attend to the tokens that come before it. This prevents the model 
#                   from “cheating” by attending to the target tokens during training.
#     Output Model: A fine-tuned CLM model can generate coherent text by predicting one token 
#                   at a time, making it suitable for text generation tasks. However, it may 
#                   not be as effective at capturing bidirectional context compared to MLM models.
# 
# Masked Language Modeling (MLM):
#     Implementation: In MLM, the model is trained to predict masked tokens within the input 
#                     sequence. During preprocessing, a certain percentage of tokens are randomly
#                     masked, and the model is trained to predict the original tokens at those 
#                     masked positions. The loss is calculated based on the model’s predictions
#                     and the actual target tokens (the original tokens that were masked).
#     Architecture: MLM is used with models like BERT, which use a bidirectional Transformer 
#                   architecture. Unlike CLM models, MLM models can attend to all tokens in 
#                   the input sequence during training, allowing them to capture context from 
#                   both left and right.
#     Output Model: A fine-tuned MLM model is better at understanding context and relationships
#                   between words in a sequence, making it suitable for tasks like text classification,
#                   sentiment analysis, named entity recognition, or question answering.
# 
# Sequence-to-Sequence (seq2seq) Modeling:
#     Implementation: In seq2seq modeling, the model is trained to generate output sequences 
#                     based on input sequences. The model consists of two parts: an encoder 
#                     that encodes the input sequence into a latent representation, and a 
#                     decoder that generates the output sequence based on this latent representation.
#                     The loss is calculated based on the model’s predictions and the actual 
#                     target output tokens.
#     Architecture: Seq2seq models typically use an encoder-decoder architecture, where both 
#                   the encoder and decoder can be based on the Transformer architecture 
#                   (e.g., T5, BART) or other architectures like LSTMs (e.g., the original seq2seq model).
#                   The encoder processes the input sequence and generates a context representation,
#                   while the decoder generates the output sequence based on the encoder’s output 
#                   and its own hidden state.
#     Output Model: A fine-tuned seq2seq model is better at tasks where the model needs to generate
#                   coherent output text based on input text, such as summarization, translation,
#                   or question answering.

# In summary, the main differences between these approaches lie in their training objectives, 
# model architectures, and the tasks they are suited for. 
# CLM models focus on predicting the next token and are good for text generation tasks, 
# MLM models focus on predicting masked tokens and are good for tasks requiring bidirectional
# context understanding, and seq2seq models focus on generating output sequences based on 
# input sequences and are good for tasks requiring the generation of coherent output text.

# MLM parameter in Huggingface selects MLM or CLM
# The mlm parameter in the DataCollatorForLanguageModeling class is a boolean flag that indicates
# whether you want to train a masked language model (MLM) or a causal language model (CLM).
# mlm=True: The data collator will prepare the training data for masked language modeling. 
# In this mode, the input sequences will have a certain percentage of tokens masked, and
# the model will be trained to predict the original tokens at those masked positions. 
# This is the training objective used by models like BERT.
# 
# mlm=False: The data collator will prepare the training data for causal language modeling, 
# also known as autoregressive language modeling. In this mode, the model will be trained to
# predict the next token in the sequence given its previous tokens. This is the training 
# objective used by models like GPT.
#
# what are the drawbacks of MLM vs CLM for a LLM chatbot?
# Each language modeling technique, Masked Language Modeling (MLM) and Causal Language Modeling
# (CLM), has its own advantages and drawbacks when used for building a chatbot. 
# Here are some key points to consider:
# 
# Masked Language Modeling (MLM):
# Pros:
#     MLM can potentially capture bidirectional context, as the model learns to predict masked 
#     tokens based on both the preceding and following tokens. This can help the model understand
#     the context more effectively, which might be useful in some chatbot scenarios.
# Cons:
#     MLM models like BERT are not designed to generate text autoregressively. While they can 
#     be fine-tuned for various NLP tasks, they are not inherently built for text generation 
#     like chatbot responses. Adapting MLM models for text generation typically requires additional
#     architectural changes, such as adding a decoder or using a seq2seq model.
#     During inference, MLM models cannot predict tokens incrementally, as the training involves 
#     predicting masked tokens in parallel. This may result in a less coherent chatbot response, 
#     as the model has not been trained to generate text sequentially.
# 
# Causal Language Modeling (CLM):
# Pros:
#     CLM models like GPT are designed for autoregressive text generation, making them more 
#     suitable for chatbot applications. They predict the next token in the sequence given 
#     the previous tokens, which aligns with how chatbot responses are generated.
#     CLM models can generate coherent and contextually relevant responses because they are 
#     trained to predict the next token in a sequence based on the preceding tokens, taking 
#     into account the context provided by the input.
# Cons:
#     CLM models do not explicitly capture bidirectional context, as they only generate tokens
#     based on the preceding tokens. This may lead to a slightly less nuanced understanding of
#     context compared to MLM models, which consider both preceding and following tokens during
#     training.
#     Due to their autoregressive nature, CLM models may have slower inference times than MLM 
#     models, especially when generating long sequences, as they have to predict each token one
#     at a time.
# 
# Which popular LLM’s were trained with DataCollatorForSeq2Seq collator?
# DataCollatorForSeq2Seq is typically used for sequence-to-sequence (seq2seq) models, where 
# the model is designed to generate output sequences based on input sequences. 
# Some popular seq2seq models in the Hugging Face Transformers library include:
# BART (Bidirectional and Auto-Regressive Transformers): BART is a denoising autoencoder 
#     with a seq2seq architecture, which has been shown to perform well on a variety of natural
#     language understanding and generation tasks. It is pre-trained using a denoising objective,
#     where the model learns to reconstruct the original text from corrupted versions of it.
#     
# T5 (Text-to-Text Transfer Transformer): T5 is a seq2seq model that frames all NLP tasks as
#     text-to-text problems. It is pre-trained using a denoising objective similar to BART. 
#     T5 has been shown to perform well on various NLP tasks, including translation, 
#     summarization, and question-answering.
#     
# MarianMT: MarianMT is a seq2seq model specifically designed for neural machine translation.
# It is part of the Marian NMT framework and has been trained on translation tasks for various
# language pairs.
# 
# Pegasus (Pre-training with Extracted Gap-sentences for Abstractive Summarization): 
# Pegasus is a seq2seq model designed for abstractive summarization. It is pre-trained using
# a gap-sentence generation task, where important sentences are removed from the input, and 
# the model learns to generate these missing sentences.
#     
# ProphetNet: ProphetNet is a seq2seq model with a novel self-supervised objective called 
# future n-gram prediction. It has been shown to perform well on tasks like abstractive 
# summarization and question generation.
# 
# Conclusion
# Understanding the differences between CLM, MLM, and Seq2Seq is crucial for selecting the 
# most appropriate training approach for your language model. By following best practices 
# and leveraging the strengths of each method, you can optimize your model’s performance 
# and achieve better results in your NLP tasks.
...
#! using fp16, we can use a batch up to 512, with bs=512, it takes 8.5g of vram and using 12 
# worker threads, we achieved 3 iter. with a batch of 64, only 3.5g of vram is used and using
# 12 workers, we get 20-21 iters per second. achieved train-loss/acc: 2.5714/38.26 val-loss/acc: 2.6665/38.16
# with adamw, batch of 64 and it took around 2:35 hours. 
# obviously it needs a much better optimization regime, but it suffices
# for our case as its just an experiment for getting familiar with image captioning
# 
trunc_len=15
...
def calculate_bleu_score(ref_caps, gen_caps):
    ...
    refs = tokenizer.batch_decode(ref_caps,to_str=True,remove_special_tokens=True)
    gens = tokenizer.batch_decode(gen_caps,to_str=True,remove_special_tokens=True)
    # calculate the bleu 
    return corpus_bleu([[ref.split()] for ref in refs], [gen for gen in gens])
...
# sidenote: extra reading
# evaluate is a python module developed by hugging face and it provides access
# to a wide range of evaluation tools. It covers a range of modalities such as
# text, computer vision, audio, etc. as well as tools to evaluate models or datasets.
# These tools are split into three categories.
# from its docs: 
# Types of evaluations
# There are different aspects of a typical machine learning pipeline that can be evaluated 
# and for each aspect 🤗 Evaluate provides a tool:
#
# Metric: 
#        A metric is used to evaluate a model's performance and usually involves the model's
#        predictions as well as some ground truth labels. 
#        You can find all integrated metrics at evaluate-metric.
# Comparison: 
#           A comparison is used to compare two models. This can for example be done by 
#           comparing their predictions to ground truth labels and computing their agreement.
#           You can find all integrated comparisons at evaluate-comparison.
# Measurement: 
#            The dataset is as important as the model trained on it. With measurements one 
#            can investigate a dataset’s properties. You can find all integrated measurements 
#            at evaluate-measurement.
# Each of these evaluation modules live on Hugging Face Hub as a Space. 
# They come with an interactive widget and a documentation card documenting its use and limitations.
#
# Each metric, comparison, and measurement is a separate Python module, but for using any of them,
# there is a single entry point: evaluate.load()
# 
...
# name clashes) you can explicitly pass the type:
...
#
#Module attributes
# All evalution modules come with a range of useful attributes that help to use a module stored 
# in a EvaluationModuleInfo object.
#
...
# First, let’s look at the description attribute of the accuracy metric:
# accuracy = evaluate.load("accuracy")
#
...
# especially if it is an academic publication you want to reference it properly. 
# For that you can look at the citation attribute:
#
# >>accuracy.citation
...
#   year={2011}
# }
# Before we can apply a metric or other evaluation module to a use-case, we need to know what the input
# format of the metric is:
# accuracy.features
# {
...
# In general we will add lists of elements so you can always think of a list around the types in 
# features. Evaluate accepts various input formats (Python lists, NumPy arrays, PyTorch tensors, etc.) 
# and converts them to an appropriate format for storage and computation.
#
# Compute
# Now that we know how the evaluation module works and what should go in there we want to actually 
# use it! When it comes to computing the actual score there are two main ways to do it:
#
#--All-in-one
#--Incremental
# 
# In the incremental approach the necessary inputs are added to the module with EvaluationModule.add()
# or EvaluationModule.add_batch() and the score is calculated at the end with EvaluationModule.compute().
# Alternatively, one can pass all the inputs at once to compute(). 
...
# {'accuracy': 0.5}
# Evaluation modules return the results in a dictionary. However, in some instances you build up 
# the predictions iteratively or in a distributed fashion in which case add() or add_batch() are 
# useful.
#
# Calculate a single metric or a batch of metrics
# In many evaluation pipelines you build the predictions iteratively such as in a for-loop. 
# In that case you could store the predictions in a list and at the end pass them to compute(). 
# With add() and add_batch() you can circumvent the step of storing the predictions separately. 
...
# different aspects of a model. E.g. for classification it is usually a good idea to compute 
# F1-score, recall, and precision in addition to accuracy to get a better picture of model performance.
# Naturally, you can load a bunch of metrics and call them sequentially. However, a more convenient way
# is to use the combine() function to bundle them together:
# 
...
#
# Evaluator
# The evaluate.evaluator() provides automated evaluation and only requires a model, dataset, metric 
# in contrast to the metrics in EvaluationModules that require the model’s predictions. 
# As such it is easier to evaluate a model on a dataset with a given metric as the inference is handled
# internally. To make that possible it uses the pipeline abstraction from transformers. 
# However, you can use your own framework as long as it follows the pipeline interface.
#
# To make an evaluation with the evaluator lets load a transformers pipeline (but you can pass your own
# custom inference class for any framework as long as it follows the pipeline call API) with an model 
# trained on IMDb, the IMDb test split and the accuracy metric.
# 
...
# Then you can create an evaluator for text classification and pass the three objects to the compute()
# method. With the label mapping evaluate provides a method to align the pipeline outputs with the 
...
# print(results)
# {'accuracy': 0.934}
# Calculating the value of the metric alone is often not enough to know if a model performs significantly
# better than another one. With bootstrapping evaluate computes confidence intervals and the standard 
# error which helps estimate how stable a score is:
#
...
# sidenote2: huggingface documentation is usually outdated, so chances are by the time you read this
# things might have changed, so be aware!
#
# Visualization
...
# trade-offs between e.g. latency and accuracy as larger models might have better performance but 
# are also slower. We are gradually adding different visualization approaches, such as plots, 
# to make choosing the best model for a use-case easier.
# For instance, if you have a list of results from multiple models (as dictionaries), you can 
# feed them into the radar_plot() function:
# 
# import evaluate
...
# plot.show()
# Which lets you visually compare the 4 models and choose the optimal one for you, 
# based on one or several metrics:
# https://huggingface.co/datasets/evaluate/media/resolve/main/example_viz.png
#
#
# Running evaluation on a suite of tasks
# It can be useful to evaluate models on a variety of different tasks to understand their 
# downstream performance. The EvaluationSuite enables evaluation of models on a collection 
# of tasks. Tasks can be constructed as (evaluator, dataset, metric) tuples and passed to 
# an EvaluationSuite stored on the Hugging Face Hub as a Space, or locally as a Python script.
# See the evaluator documentation for a list of currently supported tasks.
...
# This was a very short introduction, which I believe is important enough to put it here so you 
# read it! the evaluation section and visualization part are very important. use them!
# 
...
def calculate_scores(outputs, targets, tokenizer):
    ...
    return {**rogue_scores,
            ...
            # text generation/summarization etc as well.
            "avg-gen_len":bleu_scores["translation_length"]//len(targets)
           }
